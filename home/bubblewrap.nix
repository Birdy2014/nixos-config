...
let
  ...
  defaultArgs = {
    ...
    wayland = (mkEnvBind "WAYLAND_DISPLAY") ++ (mkRoBind "$XDG_RUNTIME_DIR/$WAYLAND_DISPLAY");

    xorg = (mkEnvBind "DISPLAY") ++ (mkRoBind "/tmp/.X11-unix");

    sound =
      (mkRoBind "$XDG_RUNTIME_DIR/pulse/native")
    ...
    dbus = (mkEnvBind "DBUS_SESSION_BUS_ADDRESS") ++ (mkRoBind "$XDG_RUNTIME_DIR/bus");
  ...
  wrapper =
    ...
    let
      ...
      desktopArgs =
        (lib.optionals sandboxConfig.allowDesktop (
          defaultArgs.desktopCommon
          ++ defaultArgs.dbus
          ++ defaultArgs.dconf
          ++ defaultArgs.wayland
          ++ defaultArgs.sound
      ...
      graphisArgs =
        if (isNull sandboxConfig.customMesaPkgsSet) then
          (lib.concatMap mkRoBind [
            "/run/opengl-driver"
        ...
        { name, ... }:
        {
          options = {
            ...
            customMesaPkgsSet = lib.mkOption {
              ...
              default = null;
              description = "Bind `/run/opengl-driver` to mesa provided by package set";
            };
